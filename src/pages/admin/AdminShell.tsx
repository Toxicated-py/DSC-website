...
export function AdminAccessDenied({ navigate }: any) {
  return (
    <div className="pt-32 pb-20 px-4 md:px-6 max-w-4xl mx-auto min-h-screen bg-background">
      <BrutalCard>
        <BrutalBadge color="bg-secondary" className="mb-4 inline-flex items-center gap-1">
        ...
        <div className="mt-6 flex gap-3 flex-wrap">
          <button
            ...
            className="px-4 py-2 border-2 border-foreground bg-white hover:bg-background transition-all font-bold uppercase tracking-widest text-xs md:text-sm"
          >
            Dashboard
          </button>
          <button
            onClick={() => navigate("/")}
            className="px-4 py-2 border-2 border-foreground bg-highlight hover:bg-white transition-all font-bold uppercase tracking-widest text-xs md:text-sm"
          >
            View Site
          </button>
        </div>
  ...
  return (
    <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
      <div>
        ...
        <p className="mt-4 font-mono text-xs md:text-sm text-muted-foreground">
...
export function AdminTabs({ visibleTabs, activeTab, openAdminTab }: any) {
  return (
    <div className="mb-8 flex gap-2 border-b-2 border-foreground pb-2 overflow-x-auto">
        ...
        <button
          ...
          className={`px-4 md:px-6 py-3 font-bold uppercase tracking-widest text-xs md:text-sm transition-all whitespace-nowrap flex items-center gap-2 ${
            activeTab === tab.id
              ...
              : "bg-white text-foreground border-2 border-transparent hover:border-foreground"
          }`}
        >
          {tab.icon}
