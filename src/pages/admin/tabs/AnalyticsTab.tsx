...
import { Check, UserCheck, Calendar, Trophy, TrendingUp, Mail, FileText, Award, Activity, Clock, MessageSquare } from "lucide-react";

import { fonts } from "../../../config/fonts";
import { BrutalBadge, BrutalCard } from "../AdminPrimitives";

export function AnalyticsTab({ ctx }: { ctx: AdminTabContext }) {
  const {
    SettingsSection,
    activeBlogs,
...
{activeTab === "analytics" && isFullAdmin && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <BrutalCard color="bg-green-500" className="text-white">
              <div className="flex items-center justify-between mb-2">
                <UserCheck size={24} />
                <Activity size={16} />
              </div>
              ...
              <div className="space-y-4">
                {memberGrowth.map((month) => (
                  <div key={month.key} className="grid grid-cols-[64px_1fr_48px] items-center gap-3">
                    ...
                    <div className="h-8 border-2 border-foreground bg-background">
                      <div className="h-full bg-primary" style={{ width: `${Math.max(6, (month.count / maxGrowth) * 100)}%` }} />
                    </div>
            ...
            <BrutalCard>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="border-2 border-foreground bg-background p-4">
                  ...
                  <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Projects submitted/published</p>
                </div>
                <div className="border-2 border-foreground bg-background p-4">
                  <FileText size={20} className="mb-3 text-violet-600" />
                  <p className="text-3xl uppercase" style={fonts.display}>{postsThisMonth}</p>
                  <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Blog posts published</p>
                </div>
                <div className="border-2 border-foreground bg-background p-4">
                  <Award size={20} className="mb-3 text-primary" />
                  <p className="text-3xl uppercase" style={fonts.display}>{issuedCertificates.length}</p>
                  <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Certificates issued</p>
                </div>
                <div className="border-2 border-foreground bg-background p-4">
                  <Mail size={20} className="mb-3 text-secondary" />
                  <p className="text-3xl uppercase" style={fonts.display}>{contactMessages.length}</p>
                  <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Contact messages</p>
                </div>
              </div>
            </BrutalCard>
          </div>
            ...
            <BrutalCard>
              ...
              <div className="space-y-4">
                {eventUtilization.map((event) => (
                  <div key={event.id} className="border-2 border-foreground bg-white p-4">
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div>
                        <p className="font-bold text-sm uppercase">{event.title}</p>
                        <p className="text-xs font-mono text-muted-foreground">{event.attendees}/{event.capacity || "unlimited"} registered - {event.checkedIn} checked in</p>
                      </div>
                      <BrutalBadge color={event.fillRate >= 90 ? "bg-secondary" : "bg-highlight"}>{event.fillRate}%</BrutalBadge>
                    </div>
                    <div className="h-4 border-2 border-foreground bg-background">
                      <div className="h-full bg-primary" style={{ width: `${event.fillRate}%` }} />
                    </div>
                  </div>
                ))}
                {eventUtilization.length === 0 && <p className="text-sm font-mono text-muted-foreground">Events will appear here after they are created.</p>}
              </div>
            </BrutalCard>
            ...
            <BrutalCard>
              ...
              <div className="space-y-3">
                {contentStatusStats.map((item) => (
                  <div key={item.label} className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-3 border-2 border-foreground bg-white p-3 text-xs font-bold uppercase tracking-widest">
                    <span>{item.label}</span>
                    <span className="text-green-500">{item.active} active</span>
                    <span className="text-secondary">{item.pending} pending</span>
                    <span className="text-muted-foreground">{item.archived} archived</span>
                  </div>
                ))}
              </div>
            </BrutalCard>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <BrutalCard>
              ...
              <div className="space-y-4">
                ...
                {popularEvents.length === 0 && <p className="text-sm font-mono text-muted-foreground">Approved events will appear here after registrations start.</p>}
              </div>
            </BrutalCard>
            ...
            <BrutalCard>
              <div className="flex items-center justify-between gap-3 mb-6">
                ...
                <button onClick={() => openAdminTab("logs")} className="px-3 py-2 border-2 border-foreground bg-white hover:bg-highlight font-bold uppercase tracking-widest text-xs">View All</button>
              </div>
              <div className="space-y-3">
                {recentAuditLogs.map((log) => (
                  <div key={log.id} className="border-2 border-foreground bg-background p-3">
                    ...
                    <p className="text-[11px] font-mono text-muted-foreground mt-2">{log.actor_email || "System"} - {log.created_at ? new Date(log.created_at).toLocaleString() : ""}</p>
                  </div>
                ))}
                {recentAuditLogs.length === 0 && <p className="text-sm font-mono text-muted-foreground">No logs yet. Run the audit log migration, then admin actions will appear here.</p>}
              </div>
            </BrutalCard>
          </div>
        </>
