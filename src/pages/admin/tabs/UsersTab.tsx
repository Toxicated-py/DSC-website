...
export function UsersTab({ ctx }: { ctx: AdminTabContext }) {
  const {
    SettingsSection,
    activeBlogs,
        ...
        <>
          ...
          <div className="mb-8 flex flex-col md:flex-row gap-4">
            ...
            <button
              ...
              className="px-6 py-3 bg-primary text-white border-2 border-foreground font-bold uppercase tracking-widest text-sm brutal-shadow brutal-shadow-hover flex items-center gap-2 justify-center"
            >
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b-2 border-foreground">
                    <th className="text-left p-4 text-xs font-bold uppercase tracking-widest text-muted-foreground">User</th>
                    <th className="text-left p-4 text-xs font-bold uppercase tracking-widest text-muted-foreground">Email</th>
                    <th className="text-left p-4 text-xs font-bold uppercase tracking-widest text-muted-foreground">Role</th>
                    <th className="text-left p-4 text-xs font-bold uppercase tracking-widest text-muted-foreground">Designation</th>
                    <th className="text-right p-4 text-xs font-bold uppercase tracking-widest text-muted-foreground">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredUsers.map((user) => (
                    <tr key={user.id} className="border-b border-slate-200 hover:bg-background transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-primary border-2 border-foreground flex items-center justify-center text-white font-bold text-sm">
                            {user.name.charAt(0)}
                          </div>
                          <span className="font-bold text-sm">{user.name}</span>
                        </div>
                      </td>
                      <td className="p-4 font-mono text-xs md:text-sm text-muted-foreground">{user.email}</td>
                      <td className="p-4">{getRoleBadge(user.role, user.verified)}</td>
                      <td className="p-4">
                        {user.designation ? (
                          <span className="text-xs font-bold uppercase tracking-widest text-primary">{user.designation}</span>
                        ) : (
                      ...
                      <td className="p-4">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            className="p-2 hover:bg-primary hover:text-white border-2 border-foreground bg-white transition-all"
                            title="Edit user"
                          >
                            <Edit size={16} />
                          </button>
                          <button
                            ...
                            className="p-2 hover:bg-secondary hover:text-white border-2 border-foreground bg-white transition-all"
