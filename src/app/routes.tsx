...
function NotFoundPage() {
  return (
    <div className="min-h-[70vh] px-6 py-24 text-center">
      ...
      <p className="mt-3 text-sm text-muted-foreground">This page does not exist or has moved.</p>
      <a href="/" className="mt-6 inline-flex border-2 border-foreground bg-highlight px-5 py-3 text-xs font-bold uppercase tracking-widest">
        Back Home
      </a>
    </div>
  );
...
function PageTitle({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <>
      <Helmet>
        <title>{title} — Data Science Club</title>
      </Helmet>
      {children}
    </>
  );
